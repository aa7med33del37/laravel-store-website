    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-10 col-sm-8 col-md-4 col-lg-4">
                ...
                <p class="phone-no"><i class="anm anm-phone-s"></i> {{ $appSetting->phone ?? '' }} - {{ $appSetting->phone2 ?? '' }}</p>
            </div>
            <div class="col-sm-4 col-md-4 col-lg-4 d-none d-lg-none d-md-block d-lg-block">
                <div class="text-center"><p class="top-header_middle-text"> {{ $appSetting->website_name ?? 'Website Name' }}</p></div>
            </div>
            <div class="col-2 col-sm-4 col-md-4 col-lg-4 text-right">
                @guest
                    ...
                    <ul class="customer-links list-inline">
                        ...
                        <li><a href="register.html">@if (Route::has('register'))
                ...
                <span class="user-menu d-block d-lg-none"><i class="anm anm-user-al" aria-hidden="true"></i></span>
                    <ul class="customer-links list-inline">
                        <li><a href="{{ url('/profile') }}" title="Profile">{{ Auth::user()->name }}</a></li>
    ...
    <div class="container-fluid">
        <div class="row align-items-center">
            ...
            <div class="logo col-md-2 col-lg-2 d-none d-lg-block">
                <a href="{{ url('/') }}">
                    <img src="{{asset($appSetting->image)}}" style="width: 57px; height: 57px;" alt="Belle Multipurpose Html Template" title="Belle Multipurpose Html Template" />
                </a>
            </div>
            <!--End Desktop Logo-->
            <div class="col-2 col-sm-3 col-md-3 col-lg-8">
                ...
                <nav class="grid__item" id="AccessibleNav"><!-- for mobile -->
                    <ul id="siteNav" class="site-nav medium center hidearrow">
                        ...
                        <li class="lvl1 parent dropdown"><a href="{{ url('/collections') }}">All Categoires<i class="anm anm-angle-down-l"></i></a>
                            <ul class="dropdown">
                                @forelse ($categories as $category)
                                    <li><a href="{{ url('collections/' . $category->slug) }}" class="site-nav">{{ $category->name }}</a></li>
                                @empty
                                @endforelse
                            </ul>
                        </li>
                        <li class="lvl1 parent megamenu"><a href="{{ url('/new-arrivals') }}">New Arrival <i class="anm anm-angle-down-l"></i></a></li>
                        <li class="lvl1 parent dropdown"><a href="{{ url('/featured-products') }}">Featured Products <i class="anm anm-angle-down-l"></i></a></li>
                  </ul>
                </nav>
                <!--End Desktop Menu-->
            </div>
            ...
            <div class="col-6 col-sm-6 col-md-6 col-lg-2 d-block d-lg-none mobile-logo">
                <div class="logo">
                    <a href="index.html">
                        <img src="{{asset('assets/frontend/images/logo.svg')}}" alt="Belle Multipurpose Html Template" title="Belle Multipurpose Html Template" />
                    </a>
                </div>
            </div>
            <!--Mobile Logo-->
            <div class="col-4 col-sm-3 col-md-3 col-lg-2">
                <div class="site-cart">
                    <a href="#" class="site-header__cart" title="Cart">
                        ...
                        <span id="CartCount" class="site-header__cart-count" data-cart-render="item_count"><livewire:frontend.cart.cart-count /></span>
                    </a>
                    <!--Minicart Popup-->
                    @auth
                    <div id="header-cart" class="block block-cart">
                            ...
                            @foreach (Auth::user()->carts as $cartItem)
                                <li class="item">
                                    <a class="product-image" href="#">
                                        <img src="{{asset($cartItem->product->productImages[0]->image)}}" alt="{{$cartItem->product->name}}" title="" />
                                    </a>
                                    <div class="product-details">
                                        <a class="pName" href="cart.html">
                                            ...
                                            <span style="color: {{$cartItem->productColor->color->code ?? 'black'}}">{{ $cartItem->productColor->color->name }}</span>
                                            @endif
                                        </a>
                                        <div class="wrapQtyBtn">
                                            <div class="qtyField">
                                                <span class="label">Qty: {{$cartItem->quantity}}</span>
                                            </div>
                                        </div>
                                            <div class="priceRow">
                                            <div class="product-price">
                                                @if ($cartItem->product->selling_price)
                                                    <span class="money">${{$cartItem->product->selling_price}}</span>
                                                @else
                                                    <span class="money">${{$cartItem->product->original_price}}</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                ...
                                @php
                                    $totalAmount += $cartItem->quantity * ($cartItem->product->selling_price ?? $cartItem->product->original_price);
                                @endphp
                            @endforeach
                        ...
                        <div class="total">
                            <div class="total-in">
                                <span class="label">Cart Subtotal:</span><span class="product-price"><span class="money">${{$totalAmount}}</span></span>
                            </div>
                ...
                <div class="site-cart" style="margin-right: 15px">
                    {{-- <a href="{{ url('/wishlist') }}"><i style="font-size: 22px;" class="fa fa-heart-o"></i><span>(0)</span></a> --}}
                    <a href="{{ url('/wishlist') }}"  title="Wishlist" style="text-decoration: none">
                        <i class="icon anm anm-heart-l" style="font-size: 22px"></i>
                        <span id="CartCount" class="site-header__cart-count" data-cart-render="item_count"><livewire:frontend.wishlist-count /></span>
                    </a>
                </div>
